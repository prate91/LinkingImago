...
LIBRARY_FILE = 'outputLibraries.json'
...
def wikiInteractive(name, wdEntities, qid, extra=''):
    ...
    for entity in wdEntities:
        ...
        label = entity["label"]["value"] if "label" in entity else 'NONE'

        country = entity["country"]["value"] if "country" in entity else 'NONE'

        gpe = entity["gpe"]["value"] if "gpe" in entity else 'NONE'
...
t = open("prova.txt", 'w', encoding='utf-8')
...
with open(TSV_FILE, encoding='utf-8') as f:
    ...
    for i, row in enumerate(tsv):
        ...
        if DATA['query']['search']:
            ...
            wdEntities = wdQuery(DATA['query']['search'][0]['title'])
    
            # vado a estrarre i dati dalla risposta chiamando wikiInteractive
            wdIRI, label, coord, country, gpe = wikiInteractive(row[0], wdEntities, DATA['query']['search'][0]['title'])
